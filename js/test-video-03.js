...
window.addEventListener("load", () => {
    ...
    // Disable the scroll trigger until our timeline is finished.
    ...
    /* CÓDIGO PARA RECOLHER ELEMENTOS HERO */
    ...
    // 🔥 Animação de cada elemento separadamente antes do scroll liberar
    timeline.to(".hero-designer", { y: -150, opacity: 0, duration: 1, ease: "power2.inOut" }, "-=0.5");
    timeline.to(".hero-title-01", { x: -200, opacity: 0, duration: 1, ease: "power2.inOut" }, "-=0.5");
    timeline.to(".hero-title-02", { x: 200, opacity: 0, duration: 1.2, ease: "power2.inOut" }, "-=0.5");
    timeline.to(".hero-description", { y: -100, opacity: 0, duration: 1, ease: "power2.inOut" }, "-=0.5");
    timeline.to("#switch-btn", { scale: 0.5, opacity: 0, duration: 1, ease: "power2.inOut" }, "-=0.5");
    timeline.to(".scroll-down-wrapper", { y: 100, opacity: 0, duration: 1, ease: "power2.inOut" }, "-=0.5");*/
...
$(".scroll_wrap").each(function () {
    ...
    paragraphs.each(function (index) {
      if (index > 0) {
        tl2.set($(this), { display: "block" });
        tl2.from($(this), { opacity: 0, y: "2rem" });
      }
      if (index < indexLength) {
        tl2.to($(this), { opacity: 0 });
        tl2.set($(this), { display: "none" });
      }
    });
  });
